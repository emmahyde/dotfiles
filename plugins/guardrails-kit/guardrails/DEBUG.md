...
Your code is not running. Verify you edited the file the process actually loads: restart the server/watcher/kernel (paste the restart), rebuild the artifact (D2), check for a twin file (~/.claude/guardrails/CODE.md C3), check the edit landed (`git diff -- <file>`).

## A previously-passing test fails after your edit
BEFORE touching the test, state in one line whether the behavior change is intended by the task. Only if intended: edit the expectation, quote old-expected vs new-expected with the justification (audited later by ~/.claude/guardrails/VERIFY.md V10). Never run snapshot-update (-u, --update-snapshots) as a first response — it converts a caught regression into a certified one.
