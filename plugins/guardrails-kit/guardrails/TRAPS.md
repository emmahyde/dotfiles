...
You are here because ~/.claude/guardrails/CODE.md C7 fired: you are touching one of the trap categories below. Find your section, follow its rows. No row covers your case and the behavior is load-bearing? Run a one-line REPL probe (`node -e` / `python -c`) and paste the output — never guess.
