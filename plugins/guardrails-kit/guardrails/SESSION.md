...
- S1. Returned from compaction or /resume? Do this FIRST, before any file-modifying tool call:
    ...
    NOT A GIT REPO (`git status` says "not a git repository")? Every `git status` / `git diff --stat HEAD` / `git diff -- <file>` step in this kit — here, S2, ~/.claude/guardrails/VERIFY.md V8, ~/.claude/guardrails/CODE.md C11, ~/.claude/guardrails/PLAN.md P5 — is replaced by: run `shasum <file>` and `ls -lT <file>` for each file before its first edit, re-run both after, and paste the pair. Never write `N/A` for a git step (an `N/A` that satisfies a checklist trains skipping the checklist).
...
## Failed attempts
<ATTEMPT entries per ~/.claude/guardrails/DEBUG.md D6>
```
...
| a fix attempt fails | Failed attempts — exact format per ~/.claude/guardrails/DEBUG.md D6 |
| you are about to run /compact, see a context-low warning, or run a command expected to emit >200 lines or take >2 min | bring Now / Next / Failed attempts current FIRST; route the output per ~/.claude/guardrails/EFFICIENCY.md E10 |
| a canonical value is established (port, path, command, version) | Facts |
- S4. After any RETURNING line, and at the first plan step after a compaction or /resume, write: `ANCHOR: goal=<original request in <=15 words> | this step serves it by <reason>`. Cannot truthfully complete the line? Stop, re-read `## Goal`, re-plan or ask. (Routine mid-plan steps no longer require an ANCHOR — v1.6 thinned it to the degradation moments it protects.)
...
- S7. `CONSTRAINT CHECK:` — before the first edit of each file while `## Constraints` is non-empty; empty or missing list -> one check per task. Exact format and procedure owned by ~/.claude/guardrails/CODE.md C4.
...
Server up, port bound, package installed, branch checked out, env var set — re-verify with one command first; the command list and the debugging entry point are owned by ~/.claude/guardrails/DEBUG.md D2.
