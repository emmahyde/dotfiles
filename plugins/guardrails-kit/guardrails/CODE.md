<!-- guardrails-kit: v1.6 | Editing this file? Read ~/.claude/guardrails/_FORMAT.md first. Never paraphrase kit text. -->
You are here because you are about to create or modify a repo source file — by Edit, Write, or a shell command that writes files — for the first time since session start or the last compaction. Prose-only .md/.txt does NOT fire this row; markdown under .claude/, skills/, or agents/ does.
...
- C4. Constraint check: print `CONSTRAINT CHECK: <path> — none apply` or `CONSTRAINT CHECK: <path> — matches '<constraint>', skipping/asking` (against docs/STATE.md `## Constraints`; no docs/STATE.md yet -> print that and create it per ~/.claude/guardrails/SESSION.md S2). `## Constraints` empty or missing -> one check per task, not per file. This format is owned here; other docs point to C4.
...
- C7. Touching any category of ~/.claude/guardrails/TRAPS.md — Dates & times, Epochs & units, Mutation vs copy, Async, Floats & money, Sort, Division & modulo, Regex & strings, Familiar-API traps, Closures in loops, Boolean logic? Read ~/.claude/guardrails/TRAPS.md and follow your rows — never guess load-bearing behavior.
...
- RS3. More than 50 hits? Re-run with a word-boundary pattern (`\b<name>\b`) excluding vendored paths (node_modules/, dist/, vendor/); still >50 -> delegate the sweep per ~/.claude/guardrails/EFFICIENCY.md E8/E9 and disposition the returned list yourself.
...
The only permitted next action on that file is a Read of the target region — never a guessed retry, never a whole-file Write. Your memory of the file is provably wrong; the file may have been changed by your own earlier edits, a formatter, or the user (re-read triggers: ~/.claude/guardrails/EFFICIENCY.md E4).
