<!-- guardrails-kit: v1.1 | Editing this file? Read ~/.claude/guardrails/_FORMAT.md first. Never paraphrase kit text. -->
...
- V10. You changed a test expectation or ran snapshot-update (-u) during this task? Paste the old-vs-new quote and justification required by ~/.claude/guardrails/DEBUG.md ("A previously-passing test fails after your edit"); absent from the transcript = FAIL.
...
Grep your production diff for any literal that also appears in the test file (strings, magic numbers, fixture ids). A match means the fix is test-shaped: remove the special case and implement the general behavior, or explicitly justify the coupling. (Invoked by ~/.claude/guardrails/DEBUG.md D8.)
...
Mark it complete ONLY in a turn where its acceptance command's passing output appears. Edit landed but nothing ran = EDITED-UNVERIFIED, never complete. Done-entry format: ~/.claude/guardrails/SESSION.md `## Done`.
