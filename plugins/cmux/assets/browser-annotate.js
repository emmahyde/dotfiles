(function () {
  if (window.__annotateOverlay) {
    if (typeof window.__annotateOverlay.__teardown === "function") {
      try {
        window.__annotateOverlay.__teardown();
      } catch (e) {}
    }
    window.__annotateOverlay.canvas.remove();
    window.__annotateOverlay.badge.remove();
    if (window.__annotateOverlay.hoverOutline)
      window.__annotateOverlay.hoverOutline.remove();
    if (window.__annotateOverlay.colorPicker)
      window.__annotateOverlay.colorPicker.remove();
    window.__annotateOverlay = null;
  }

  // Tracked so re-injection can call removeEventListener — window/document
  // listeners outlive the IIFE scope and would otherwise swallow page keys.
  var teardowns = [];
  ...
  function redraw() {
    ...
    for (var j = 0; j < selections.length; j++) {
      ...
      var rx = r.x + window.scrollX,
        ry = r.y + window.scrollY;
  ...
  function updateBadge() {
    if (!enabled) {
      ...
      badge.innerHTML =
        '<div style="padding:6px 12px;opacity:0.7">Annotate OFF \u2014 Ctrl+D</div>';
      return;
    }
    ...
    var toolBtns = tools
      .map(function (m) {
        ...
        var border = active
          ? "1px solid " + modeColors[m]
          : "1px solid rgba(255,255,255,0.15)";
        var parts = toolNames[m];
        return (
          '<span class="ann-tool" data-mode="' +
          m +
    ...
    var swatches = COLORS.map(function (c) {
      var sel = c === color;
      return (
        '<span class="ann-color" data-color="' +
        c +
        ...
        ";" +
        "border:" +
        (sel ? "2px solid #fff" : "1px solid rgba(255,255,255,0.25)") +
        ";" +
    ...
    var actions =
      "Z Undo \u00B7 R Redo \u00B7 C Copy \u00B7 Q Clear \u00B7 Ctrl+D Off";

    badge.innerHTML =
      '<div style="padding:6px 10px 4px">' +
      toolBtns +
      "</div>" +
      '<div style="padding:2px 10px 4px;display:flex;align-items:center;gap:2px">' +
      swatches +
      "</div>" +
      '<div style="padding:2px 10px 6px;font-size:10px;opacity:0.5">' +
      actions +
      "</div>";
  }
  updateBadge();
    ...
    function (e) {
      ...
      if (existingIdx >= 0) {
        ...
        for (var k = 0; k < selections.length; k++) {
          ...
          if (selections[k]._el)
            selections[k]._el.setAttribute("data-sel", k + 1);
        }
        redraw();
        return;
      }
  ...
  function showTextInput(clientX, clientY) {
    ...
    textInput.placeholder =
      "Type here \u2014 Shift+Enter for newline, Enter to place";
    ...
    function finish(val) {
      ...
      canvas.style.pointerEvents =
        enabled && mode !== "select" ? "auto" : "none";
    }
    function commit() { finish(textInput.value.trim()); }
  ...
  canvas.addEventListener("mousedown", function (e) {
    ...
    if (idx >= 0) {
      moveTarget = {
        opIndex: idx,
        startX: pageX(e.clientX),
        startY: pageY(e.clientY),
      };
      drawing = true;
    ...
    if (mode === "freehand") {
      currentStroke = {
        type: "freehand",
        points: [{ x: pageX(e.clientX), y: pageY(e.clientY) }],
        color: color,
      };
  ...
  canvas.addEventListener("mousemove", function (e) {
    ...
    if (moveTarget && drawing) {
      var nowX = pageX(e.clientX),
        nowY = pageY(e.clientY);
      var mdx = nowX - moveTarget.startX,
        mdy = nowY - moveTarget.startY;
    ...
    if (!drawing && mode !== "text") {
      ...
      if (idx !== moveHover) {
        moveHover = idx;
        canvas.style.cursor =
          idx >= 0 ? "grab" : mode === "text" ? "text" : "crosshair";
        redraw();
      }
      return;
    }
    ...
    switch (key) {
      case "q":
        ...
        selections.forEach(function (s) {
          if (s._el) s._el.removeAttribute("data-sel");
        });
        operations.length = 0;
        redoStack.length = 0;
        selections.length = 0;
      ...
      case "z":
        ...
        if (
          ...
          if (removedSel && removedSel._el)
            removedSel._el.removeAttribute("data-sel");
      ...
      case "c":
        ...
        navigator.clipboard
          ...
          .then(function () {
            badge.innerHTML =
              '<span style="color:#22aa44;font-weight:700">Copied ' +
              selections.length +
              " selections!</span>";
          ...
          .catch(function () {
            badge.innerHTML =
              '<span style="color:#ff2222;font-weight:700">Copy failed — clipboard blocked</span>';
