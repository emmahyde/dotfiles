...
|---|---|---|
| Find code by meaning | lumen `semantic_search` | High (embeddings are language-agnostic) |
...
| Callers / callees | `grepai trace callers <name>` | Medium — name collisions on common methods |
...
Default to **ast-grep for structure** and **lumen for meaning** on Ruby — the two axes that don't depend on type resolution — and reach for Sorbet/ruby-lsp only when a relationship answer must be exact.

## Quick reference

- `ast-grep --lang ruby` — structural search/rewrite (most reliable structural tool here)
- lumen `semantic_search` — meaning-based discovery
- `sem impact` / `grepai trace` — relationships, but **verify** the edges
