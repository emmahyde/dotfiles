...
| Need | Command | Notes |
|---|---|---|
| Impact / blast radius (what breaks if X changes) | `sem impact <Entity>` | Transitive, includes affected tests. Resolves by `module::class::method` — disambiguates overloads (`sem impact Generate` lists all 16 and asks you to pick via `--file`/`--entity-id`). |
| Direct callers / callees | `grepai trace callers <Symbol>` | Name/AST-based — catches the C# edges import graphs miss. |
...
| Find code by meaning | lumen `semantic_search` | Embeddings are language-agnostic; works fine. |
...
- `grepai trace callers <Symbol>` — call edges
