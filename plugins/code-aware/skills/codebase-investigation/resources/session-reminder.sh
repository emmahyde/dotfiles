...
# Resolve the reminder file relative to this script, so it works whether the
# skill is installed globally (~/.claude/skills/...) or shipped inside a plugin
# (the SessionStart hook invokes us via $CLAUDE_PLUGIN_ROOT).
here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
reminder="$here/reminder.md"
