...
This machine has a code-intelligence stack (lumen · sem · grepai · ast-grep · lizard) that beats grep/Read for understanding and navigating code. When a task matches a trigger below, invoke the **`codebase-investigation`** skill BEFORE reaching for Grep/Glob/Read:

- "Where is X implemented?" / "How does Y work?" → semantic search (lumen), not grep
- "What breaks if I change Z?" / "Who calls this?" → impact + call-graph (sem / grepai), not grep
