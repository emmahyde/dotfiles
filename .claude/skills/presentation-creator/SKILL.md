...
description: Create data-driven presentation slides using React, Vite, and Recharts with a clean, minimal design system. Use when asked to "create a presentation", "build slides", "make a deck", "create a data presentation". Scaffolds a complete slide-based app with charts, animations, and single-file HTML output.
---

# Presentation Builder

Create interactive, data-driven presentation slides using React + Vite + Recharts, styled with the built-in design system and built as a single distributable HTML file.
...
5. Any brand colors or a logo to use? (optional — defaults to the built-in neutral palette and a plain text wordmark if none supplied)
...
Read `references/design-system.md` for the complete default color palette, typography, CSS variables, layout utilities, and animation system.
...
function App() {
  ...
  return (
    <>
      {cur > 0 && hasBrandGlyph && <div className="glyph-watermark"><BrandGlyph size={50} /><span className="watermark-title">TITLE</span></div>}
...
## Step 5: Style with the Design System

Apply the complete CSS from the design system reference. Key elements:

- **Font**: Rubik from Google Fonts
- **Colors**: CSS variables for UI chrome (`--accent`, `--dark`, `--muted`). Semantic CSS variables (`--semantic-green`, `--semantic-red`, `--semantic-amber`) only where color conveys meaning. Categorical palette (`CAT[]`) for all other data visualization.
...
- **Tags**: `.tag-accent`, `.tag-red`, `.tag-green`, `.tag-amber` for slide labels
- **Logo**: Only if the user supplied a brand logo/glyph SVG — read its exact contents and embed the path data as-is, never approximate a trademarked mark. With no logo supplied, use a plain text wordmark on the title slide and skip the watermark.

## Step 6: Common Slide Patterns

### Title Slide
Optional user-supplied logo (falls back to a plain text wordmark) + h1 + subtitle + author/date info.
...
- Uses the default design system (colors, fonts, icons), or the user's brand palette/logo when supplied
