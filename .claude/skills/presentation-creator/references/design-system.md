...
These are a neutral default theme — swap `--accent`/`--accent-light`/`--accent-bg` (and `--dark` if
it should match) for the user's actual brand color when one is supplied in Step 1.

```css
:root {
  /* ── UI chrome (placeholder default — replace with brand colors if supplied) ── */
  --dark: #18181b;
  --accent: #4f46e5;
  --accent-light: #a5b4fc;
  --accent-bg: #eef2ff;
...
.progress {
  ...
  background: var(--accent); transition: width 0.3s; z-index: 10;
}
...
Only when the user supplied a brand glyph in Step 1: every slide shows it small and low-contrast in
the top-left corner, using its exact path data. If no glyph was supplied, skip this entirely — no
watermark, no placeholder mark.
...
Only relevant if the user supplied a brand logo/glyph SVG in Step 1. **Never approximate or
reconstruct a trademarked logo from memory** — only embed a mark the user actually provided, reading
its file contents and extracting the exact `<path>` data:

```jsx
// Read the user-supplied SVG file, extract the content, and embed it:
function BrandLogo({ width = 120 }) {
  // The SVG content should be read from the user-supplied logo file
  // and embedded directly — never use a hardcoded approximation.
  return (
    <svg viewBox="0 0 200 44" width={width} fill="none" aria-hidden="true">
      {/* paste exact path data from the supplied logo SVG */}
    </svg>
  );
}
...
function BrandGlyph({ size = 32 }) {
  return (
    <svg viewBox="0 0 72 66" width={size} height={size} aria-hidden="true">
      {/* paste exact path data from the supplied glyph SVG */}
    </svg>
  );
}
```

If no logo/glyph was supplied, use a plain text wordmark on the title slide and omit the watermark
entirely — do not invent placeholder brand art.
...
```css
.wrapup-col--muted { border-top: 3px solid var(--muted); }
.wrapup-col--muted h3 { color: var(--dark); }
