...
```
Copy
You can only initialize an app once. The Railtie [initializers](https://guides.rubyonrails.org/configuring.html#initializers) are run through the `run_initializers` method which is defined in `railties/lib/rails/initializable.rb`:
...
Do not confuse Railtie initializers overall with the [load_config_initializers](https://guides.rubyonrails.org/configuring.html#using-initializer-files) initializer instance or its associated config initializers in `config/initializers`.
