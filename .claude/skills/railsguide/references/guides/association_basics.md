...
When you set up an association, Rails helps define and manage the [Primary Key](https://en.wikipedia.org/wiki/Primary_key) and [Foreign Key](https://en.wikipedia.org/wiki/Foreign_key) relationships between instances of the two models, while the database ensures that your data stays consistent and properly linked.
...
```
Copy
To delete an author and ensure all their books are also deleted, you'd need to retrieve all the author's `books`, loop through each `book` to destroy it, and then destroy the author.
...
```
Copy
When you set up an association in Rails, you still need to create a [migration](https://guides.rubyonrails.org/active_record_migrations.html) to ensure that the database is properly configured to handle the association. This migration will need to add the necessary foreign key columns to your database tables.
...
### 2.1. `belongs_to`
A [`belongs_to`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html#method-i-belongs_to) association sets up a relationship with another model, such that each instance of the declaring model "belongs to" one instance of the other model. For example, if your application includes authors and books, and each book can be assigned to exactly one author, you'd declare the book model this way:
...
```
Copy
In database terms, the `belongs_to` association says that this model's table contains a column which represents a reference to another table. This can be used to set up one-to-one or one-to-many relations, depending on the setup. If the table _of the other class_ contains the reference in a one-to-one relation, then you should use `has_one` instead.
When used alone, `belongs_to` produces a one-directional one-to-one relationship. Therefore each book in the above example "knows" its author, but the authors don't know about their books. To set up a bi-directional association - use `belongs_to` in combination with a `has_one` or `has_many` on the other model, in this case the Author model.
By default `belongs_to` validates the presence of the associated record to guarantee reference consistency.If `optional` is set to true in the model, then `belongs_to` does not guarantee reference consistency. This means that the foreign key in one table might not reliably point to a valid primary key in the referenced table.
...
We'll discuss some of the common methods, but you can find an exhaustive list in the [ActiveRecord Associations API](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html#method-i-belongs_to).
...
```
Copy
Do not confuse `model.association_changed?` with `model.association.changed?`. The former checks if the association has been replaced with a new record, while the latter tracks changes to the attributes of the association.
...
Assigning an object to a `belongs_to` association does _not_ automatically save either the current object or the associated object. However, when you save the current object, the association is saved as well.

### 2.2. `has_one`
A [`has_one`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html#method-i-has_one) association indicates that one other model has a reference to this model. That model can be fetched through this association.
...
We'll discuss some of the common methods, but you can find an exhaustive list in the [ActiveRecord Associations API](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html#method-i-has_one).
...
### 2.3. `has_many`
A [`has_many`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html#method-i-has_many) association is similar to `has_one`, but indicates a one-to-many relationship with another model. You'll often find this association on the "other side" of a `belongs_to` association. This association indicates that each instance of the model has zero or more instances of another model. For example, in an application containing authors and books, the author model could be declared like this:
...
If you wish to enforce [referential integrity](https://en.wikipedia.org/wiki/Referential_integrity) at the database level, add the [`foreign_key: true`](https://guides.rubyonrails.org/active_record_migrations.html#foreign-keys) option to the `reference` column declarations above. This will ensure that the `author_id` in the books table must correspond to a valid `id` in the `authors` table,
...
When you declare a `has_many` association, the declaring class gains numerous methods related to the association. Some of these are:
  ...
  * [`collection.create!(attributes = {})`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-create-21)

We'll discuss some of the common methods, but you can find an exhaustive list in the [ActiveRecord Associations API](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html#method-i-has_many).
...
```
Copy
The [`collection.delete`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-delete) method removes one or more objects from the collection by setting their foreign keys to `NULL`.
...
The [`collection.destroy`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-destroy) method removes one or more objects from the collection by running `destroy` on each object.
...
The [`collection.reload`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-reload) method returns a Relation of all of the associated objects, forcing a database read. If there are no associated objects, it returns an empty Relation.
...
```
Copy
The [`collection.empty?`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-empty-3F) method returns `true` if the collection does not contain any associated objects.
...
```
Copy
The [`collection.size`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-size) method returns the number of objects in the collection.
...
```
Copy
The [`collection.find`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-find) method finds objects within the collection's table.
...
```
Copy
The [`collection.where`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-where) method finds objects within the collection based on the conditions supplied but the objects are loaded lazily meaning that the database is queried only when the object(s) are accessed.
...
```
Copy
The [`collection.exists?`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-exists-3F) method checks whether an object meeting the supplied conditions exists in the collection's table.

##### 2.3.1.4. Building and Creating Associated Objects
The [`collection.build`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-build) method returns a single or array of new objects of the associated type. The object(s) will be instantiated from the passed attributes, and the link through their foreign key will be created, but the associated objects will _not_ yet be saved.
...
A [`has_many :through`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html#method-i-has_many) association is often used to set up a many-to-many relationship with another model. This association indicates that the declaring model can be matched with zero or more instances of another model by proceeding _through_ an intermediate "join" model.
...
We call `Physician.appointments` and `Appointment.patient` the _through_ and _source_ associations of `Physician.patients`, respectively.
The corresponding migration might look like this:
...
You could also consider using a [composite primary key](https://guides.rubyonrails.org/active_record_composite_primary_keys.html) for the join table in the `has_many :through` relationship like below:
...
Automatic deletion of join models is direct, no destroy callbacks are triggered. You can read more about callbacks in the [Active Record Callbacks Guide](https://guides.rubyonrails.org/active_record_callbacks.html).
...
### 2.5. `has_one :through`
A [`has_one :through`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html#method-i-has_one) association sets up a one-to-one relationship with another model through an intermediary model. This association indicates that the declaring model can be matched with one instance of another model by proceeding _through_ a third model.
...
We call `Supplier.account` and `Account.account_history` the _through_ and _source_ associations of `Supplier.account_history`, respectively.
...
```
Copy
The through association must be a `has_one`, `has_one :through`, or non-polymorphic `belongs_to`. That is, a non-polymorphic singular association. On the other hand, source associations can be polymorphic as long as you provide a source type.

### 2.6. `has_and_belongs_to_many`
A [`has_and_belongs_to_many`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html#method-i-has_and_belongs_to_many) association creates a direct many-to-many relationship with another model, with no intervening model. This association indicates that each instance of the declaring model refers to zero or more instances of another model.
...
We'll discuss some of the common methods, but you can find an exhaustive list in the [ActiveRecord Associations API](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html#method-i-has_and_belongs_to_many).
...
```
Copy
The [`collection<<`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-3C-3C) method adds one or more objects to the collection by creating records in the join table.
...
The [`collection.delete`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-delete) method removes one or more objects from the collection by deleting records in the join table. This does not destroy the objects.
...
```
Copy
The [`collection.destroy`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-destroy) method removes one or more objects from the collection by deleting records in the join table. This does not destroy the objects.
...
```
Copy
The [`collection.clear`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/CollectionProxy.html#method-i-clear) method removes every object from the collection by deleting the rows from the joining table. This does not destroy the associated objects.
...
You'd use `has_many :through` when:
  ...
  * You require [validations](https://guides.rubyonrails.org/active_record_validations.html) or [callbacks](https://guides.rubyonrails.org/active_record_callbacks.html) on the join model.
...
To set up a polymorphic association manually you would need to declare both a foreign key column (`imageable_id`) and a type column (`imageable_type`) in the model:
...
While creating the polymorphic association manually is acceptable, it is instead recommended to use `t.references` or its alias `t.belongs_to` and specify `polymorphic: true` so that Rails knows that the association is polymorphic, and it automatically adds both the foreign key and type columns to the table.
...
If you're working with composite primary keys in your Rails models and need to ensure the correct handling of associations, please refer to the [Associations section of the Composite Primary Keys guide](https://guides.rubyonrails.org/active_record_composite_primary_keys.html#associations-between-models-with-composite-primary-keys). This section provides comprehensive guidance on setting up and using associations with composite primary keys in Rails, including how to specify composite foreign keys when necessary.
...
The `to_table` option passed to `foreign_key` and more are explained in [`SchemaStatements#add_reference`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/ConnectionAdapters/SchemaStatements.html#method-i-add_reference).
...
There may be cases (like when working with a legacy database) where you need to override the name of the inheritance column. This can be achieved with the [inheritance_column](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/ModelSchema.html#method-c-inheritance_column) method.
...
There may be cases (like when working with a legacy database) where you need to disable Single Table Inheritance altogether. If you don't disable STI properly, you might encounter an [`ActiveRecord::SubclassNotFound`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/SubclassNotFound.html) error.
To disable STI, you can set the [inheritance_column](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/ModelSchema.html#method-c-inheritance_column) to `nil`.
...
We can enhance our `Entry` delegator by defining `delegate` and using polymorphism on the subclasses. For example, to delegate the `title` method from `Entry` to its subclasses:
...
But what if you want to reload the cache, because data might have been changed by some other part of the application? Just call [`reload`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Relation.html#method-i-reload) on the association:
...
```
Copy
You can read more about the `create_join_table` method in the [Active Record Migration Guides](https://guides.rubyonrails.org/active_record_migrations.html#creating-a-join-table)
...
Since only one copy of the `Author` object is loaded, it helps to prevent inconsistencies.
...
```
Copy
  * Validate the [presence](https://guides.rubyonrails.org/active_record_validations.html#presence) and [absence](https://guides.rubyonrails.org/active_record_validations.html#absence) of associations in more cases:
...
Custom scopes on the opposite association also prevent automatic identification, as do custom scopes on the association itself unless [`config.active_record.automatic_scope_inversing`](https://guides.rubyonrails.org/configuring.html#config-active-record-automatic-scope-inversing) is set to true.
...
```
Copy
Each association supports numerous options which you can read more about in [`Options` section of each association in the ActiveRecord Associations API](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Associations/ClassMethods.html). We'll discuss some of the common use cases below.
...
The `:dependent` option is ignored with the `:through` option. When using `:through`, the join model must have a `belongs_to` association, and the deletion affects only the join records, not the associated records.
...
Three reporting modes are supported (`:warn`, `:raise`, and `:notify`), and backtraces can be enabled or disabled. Defaults are `:warn` mode and disabled backtraces.
Please, check the documentation of `ActiveRecord::Associations::ClassMethods` for further details.
...
You can use any of the standard [querying methods](https://guides.rubyonrails.org/active_record_querying.html) inside the scope block. The following ones are discussed below:
  * `where`
  * `includes`
  * `readonly`
  * `select`
...
If you use the `select` method on a `belongs_to` association, you should also set the `:foreign_key` option to guarantee correct results. For example:
...
```
Copy
By default, querying `author.books.size` results in a database call to perform a `COUNT(*)` query. To optimize this, you can add a counter cache to the _belonging_ model (in this case, `Book`). This way, Rails can return the count directly from the cache without querying the database.
...
If for some reason you change the value of an owner model's primary key, and do not also update the foreign keys of the counted models, then the counter cache may have stale data. In other words, any orphaned models will still count towards the counter. To fix a stale counter cache, use [`reset_counters`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/CounterCache/ClassMethods.html#method-i-reset_counters).

### 8.4. Callbacks
[Normal callbacks](https://guides.rubyonrails.org/active_record_callbacks.html) hook into the life cycle of Active Record objects, allowing you to work with those objects at various points. For example, you can use a `:before_save` callback to cause something to happen just before an object is saved.
...
Read more about association callbacks in the [Active Record Callbacks Guide](https://guides.rubyonrails.org/active_record_callbacks.html#association-callbacks)
