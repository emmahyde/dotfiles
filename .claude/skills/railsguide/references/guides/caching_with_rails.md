...
By default, Action Controller caching is only enabled in your production environment. You can play around with caching locally by running `bin/rails dev:cache`, or by setting [`config.action_controller.perform_caching`](https://guides.rubyonrails.org/configuring.html#config-action-controller-perform-caching) to `true` in `config/environments/development.rb`.
Changing the value of `config.action_controller.perform_caching` will only have an effect on the caching provided by Action Controller. For instance, it will not impact low-level caching, that we address below.
...
Most template dependencies can be derived from calls to `render` in the template itself. Here are some examples of render calls that [`ActionView::Digestor`](https://api.rubyonrails.org/v8.1.3/classes/ActionView/Digestor.html) knows how to decode:
...
```
Copy
If `database` or `databases` is not specified in the cache configuration, Solid Cache will use the ActiveRecord::Base connection pool. This means that cache reads and writes will be part of any wrapping database transaction.
...
By default, caching is _enabled_ in development mode with `:memory_store`. This doesn't apply to Action Controller caching, which is disabled by default.
...
```
Copy
To disable caching set `cache_store` to `:null_store`
...
[`ActiveSupport::Cache::Store`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Cache/Store.html) provides the foundation for interacting with the cache in Rails. This is an abstract class, and you cannot use it on its own. Instead, you must use a concrete implementation of the class tied to a storage engine. Rails ships with several implementations, documented below.
The main API methods are [`read`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Cache/Store.html#method-i-read), [`write`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Cache/Store.html#method-i-write), [`delete`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Cache/Store.html#method-i-delete), [`exist?`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Cache/Store.html#method-i-exist-3F), and [`fetch`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Cache/Store.html#method-i-fetch).
...
[`ActiveSupport::Cache::MemoryStore`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Cache/MemoryStore.html) keeps entries in memory in the same Ruby process. The cache store has a bounded size specified by sending the `:size` option to the initializer (default is 32Mb). When the cache exceeds the allotted size, a cleanup will occur and the least recently used entries will be removed.
...
[`ActiveSupport::Cache::FileStore`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Cache/FileStore.html) uses the file system to store entries. The path to the directory where the store files will be stored must be specified when initializing the cache.
...
The [`write`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Cache/MemCacheStore.html#method-i-write) (and `fetch`) method on this cache accepts additional options that take advantage of features specific to memcached.

### 5.6. `ActiveSupport::Cache::RedisCacheStore`
[`ActiveSupport::Cache::RedisCacheStore`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Cache/RedisCacheStore.html) takes advantage of Redis support for automatic eviction when it reaches max memory, allowing it to behave much like a Memcached cache server.
