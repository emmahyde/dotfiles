...
Finder methods that return a collection, such as `where` and `group`, return an instance of [`ActiveRecord::Relation`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Relation.html). Methods that find a single entity, such as `find` and `first`, return a single instance of the model.
...
#### 2.1.1. `find`
Using the [`find`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-find) method, you can retrieve the object corresponding to the specified _primary key_ that matches any supplied options. For example:
...
#### 2.1.2. `take`
The [`take`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-take) method retrieves a record without any implicit ordering. For example:
...
```
Copy
The [`take!`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-take-21) method behaves exactly like `take`, except that it will raise `ActiveRecord::RecordNotFound` if no matching record is found.
The retrieved record may vary depending on the database engine.

#### 2.1.3. `first`
The [`first`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-first) method finds the first record ordered by primary key (default). For example:
...
```
Copy
The [`first!`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-first-21) method behaves exactly like `first`, except that it will raise `ActiveRecord::RecordNotFound` if no matching record is found.

#### 2.1.4. `last`
The [`last`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-last) method finds the last record ordered by primary key (default). For example:
...
```
Copy
The [`last!`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-last-21) method behaves exactly like `last`, except that it will raise `ActiveRecord::RecordNotFound` if no matching record is found.

#### 2.1.5. `find_by`
The [`find_by`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-find_by) method finds the first record matching some conditions. For example:
...
The [`find_by!`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-find_by-21) method behaves exactly like `find_by`, except that it will raise `ActiveRecord::RecordNotFound` if no matching record is found. For example:
...
When specifying conditions on methods like [`find_by`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-find_by) and [`where`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-where), the use of `id` will match against an `:id` attribute on the model. This is different from [`find`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/FinderMethods.html#method-i-find), where the ID passed in should be a primary key value.
...
```
Copy
Here, we might intend to search for a single record with the composite primary key `[5, 10]`, but Active Record will search for a record with an `:id` column of _either_ 5 or 10, and may return the wrong record.
The [`id_value`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/ModelSchema.html#method-i-id_value) method can be used to fetch the value of the `:id` column for a record, for use in finder methods such as `find_by` and `where`. See example below:
...
#### 2.2.1. `find_each`
The [`find_each`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Batches.html#method-i-find_each) method retrieves records in batches and then yields _each_ one to the block. In the following example, `find_each` retrieves customers in batches of 1000 and yields them to the block one by one:
...
## 3. Conditions
The [`where`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-where) method allows you to specify conditions to limit the records returned, representing the `WHERE`-part of the SQL statement. Conditions can either be specified as a string, array, or hash.
...
To avoid these problems, use [`sanitize_sql_like`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Sanitization/ClassMethods.html#method-i-sanitize_sql_like) to escape wildcard characters in the relevant portion of the argument:
...
### 3.4. NOT Conditions
`NOT` SQL queries can be built by [`where.not`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods/WhereChain.html#method-i-not):
...
### 3.5. OR Conditions
`OR` conditions between two relations can be built by calling [`or`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-or) on the first relation, and passing the second one as an argument.
...
```
Copy
`AND` conditions for the logical intersection between relations can be built by calling [`and`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-and) on the first relation, and passing the second one as an argument.
...
## 4. Ordering
To retrieve records from the database in a specific order, you can use the [`order`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-order) method.
...
To select only a subset of fields from the result set, you can specify the subset via the [`select`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-select) method.
...
If you would like to only grab a single record per unique value in a certain field, you can use [`distinct`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-distinct):
...
## 6. Limit and Offset
To apply `LIMIT` to the SQL fired by the `Model.find`, you can specify the `LIMIT` using [`limit`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-limit) and [`offset`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-offset) methods on the relation.
...
## 7. Grouping
To apply a `GROUP BY` clause to the SQL fired by the finder, you can use the [`group`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-group) method.
...
To get the total of grouped items on a single query, call [`count`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Calculations.html#method-i-count) after the `group`.
...
SQL uses the `HAVING` clause to specify conditions on the `GROUP BY` fields. You can add the `HAVING` clause to the SQL fired by the `Model.find` by adding the [`having`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-having) method to the find.
For example:
...
### 8.1. `unscope`
You can specify certain conditions to be removed using the [`unscope`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-unscope) method. For example:
...
### 8.2. `only`
You can also override conditions using the [`only`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/SpawnMethods.html#method-i-only) method. For example:
...
### 8.3. `reselect`
The [`reselect`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-reselect) method overrides an existing select statement. For example:
...
### 8.4. `reorder`
The [`reorder`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-reorder) method overrides the default scope order. For example, if the class definition includes this:
...
### 8.5. `reverse_order`
The [`reverse_order`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-reverse_order) method reverses the ordering clause if specified.
...
### 8.6. `rewhere`
The [`rewhere`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-rewhere) method overrides an existing, named `where` condition. For example:
...
### 8.7. `regroup`
The [`regroup`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-regroup) method overrides an existing, named `group` condition. For example:
...
### 12.1. `joins`
There are multiple ways to use the [`joins`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-joins) method.
...
```
Copy
For more advanced conditions or to reuse an existing named scope, [`merge`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/SpawnMethods.html#method-i-merge) may be used. First, let's add a new named scope to the `Order` model:
...
If you want to select a set of records whether or not they have associated records you can use the [`left_outer_joins`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-left_outer_joins) method.
...
### 13.5. `strict_loading`
Eager loading can prevent N + 1 queries but you might still be lazy loading some associations. To make sure no associations are lazy loaded you can enable [`strict_loading`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/QueryMethods.html#method-i-strict_loading).
...
```
Copy
To enable for all relations, change the [`config.active_record.strict_loading_by_default`](https://guides.rubyonrails.org/configuring.html#config-active-record-strict-loading-by-default) flag to `true`.
To send violations to the logger instead, change [`config.active_record.action_on_strict_loading_violation`](https://guides.rubyonrails.org/configuring.html#config-active-record-action-on-strict-loading-violation) to `:log`.

### 13.6. `strict_loading!`
We can also enable strict loading on the record itself by calling [`strict_loading!`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Core.html#method-i-strict_loading-21):
...
To define a simple scope, we use the [`scope`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Scoping/Named/ClassMethods.html#method-i-scope) method inside the class, passing the query that we'd like to run when this scope is called:
...
If we wish for a scope to be applied across all queries to the model we can use the [`default_scope`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Scoping/Default/ClassMethods.html#method-i-default_scope) method within the model itself.
...
```
Copy
If we do want the last `where` clause to win then [`merge`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/SpawnMethods.html#method-i-merge) can be used.
...
If we wish to remove scoping for any reason we can use the [`unscoped`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Scoping/Default/ClassMethods.html#method-i-unscoped) method. This is especially useful if a `default_scope` is specified in the model and should not be applied for this particular query.
...
For example, given this [`enum`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Enum.html#method-i-enum) declaration:
...
```
Copy
Full documentation about enums can be found [here](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Enum.html).

## 17. Understanding Method Chaining
The Active Record pattern implements [Method Chaining](https://en.wikipedia.org/wiki/Method_chaining), which allows us to use multiple Active Record methods together in a simple and straightforward way.
You can chain methods in a statement when the previous method called returns an [`ActiveRecord::Relation`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Relation.html), like `all`, `where`, and `joins`. Methods that return a single object (see Retrieving a Single Object Section) have to be at the end of the statement.
...
The [`find_or_create_by`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Relation.html#method-i-find_or_create_by) method checks whether a record with the specified attributes exists. If it doesn't, then `create` is called. Let's see an example.
...
You can also use [`find_or_create_by!`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Relation.html#method-i-find_or_create_by-21) to raise an exception if the new record is invalid. Validations are not covered on this guide, but let's assume for a moment that you temporarily add
...
### 19.4. `ids`
[`ids`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Calculations.html#method-i-ids) can be used to pluck all the IDs for the relation using the table's primary key.
...
## 21. Calculations
This section uses [`count`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Calculations.html#method-i-count) as an example method in this preamble, but the options described apply to all sub-sections.
...
### 21.2. `average`
If you want to see the average of a certain number in one of your tables you can call the [`average`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Calculations.html#method-i-average) method on the class that relates to the table. This method call will look something like this:
...
### 21.3. `minimum`
If you want to find the minimum value of a field in your table you can call the [`minimum`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Calculations.html#method-i-minimum) method on the class that relates to the table. This method call will look something like this:
...
### 21.4. `maximum`
If you want to find the maximum value of a field in your table you can call the [`maximum`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Calculations.html#method-i-maximum) method on the class that relates to the table. This method call will look something like this:

```
Order.maximum("subtotal")

### 21.5. `sum`
If you want to find the sum of a field for all records in your table you can call the [`sum`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Calculations.html#method-i-sum) method on the class that relates to the table. This method call will look something like this:

```
Order.sum("subtotal")

## 22. Running EXPLAIN
You can run [`explain`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Relation.html#method-i-explain) on a relation. EXPLAIN output varies for each database.
For example, running:
