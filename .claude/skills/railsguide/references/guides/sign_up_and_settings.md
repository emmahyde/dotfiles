...
```
Copy
`has_secure_password` only validates the presence of the password. Consider adding more validations for password minimum length or complexity to improve security.
...
### 2.3. Requiring Unauthenticated Access
...
Rails makes this easy with the [`rate_limit`](https://api.rubyonrails.org/v8.1.3/classes/ActionController/RateLimiting/ClassMethods.html) method in controllers.
...
`Current.user` comes from [CurrentAttributes](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/CurrentAttributes.html) which is a per-request attribute which resets automatically before and after each request. The Rails authentication generator uses this to keep track of the logged in User.
...
activerecord:
    attributes:
      user:
        password_challenge: "Currentpassword"
...
### 4.3. Settings Layout
While we're here, let's add a new layout for Settings so we can organize them in a sidebar. To do this, we're going to use a [Nested Layout](https://guides.rubyonrails.org/layouts_and_rendering.html#using-nested-layouts).
...
To do this, add the `.navbar` class to these selectors in `app/assets/stylesheets/application.css`.
...
Occasionally, users need to change the email address on their account. To do this safely, we need to store the new email address and send an email to confirm the change.
...
```
Copy
Test out this process by navigating to <http://localhost:3000/settings/email> and updating your email address. Watch the Rails server logs for the email contents and open the confirm link in your browser to update the email in the database.
...
```
Copy
When `admin` is read-only, we have to directly update this in the database instead of using Active Record.
...
### 9.2. Clean Up Public Product Views
...
### 9.4. Updating Admin Product Views
The admin views need some tweaks to work inside the `store` namespace.
First, let's fix the form in `app/views/store/products/_form.html.erb` by updating the `model:` argument to use the `store` namespace.
...
```
Copy
Then add first and last names to the fixtures in `test/fixtures/users.yml` to pass validations:
...
Since we previously setup Kamal in the [Getting Started Guide](https://guides.rubyonrails.org/getting_started.html#deploying-to-production), we just need to push our code changes to our Git repository and run:
...
Next, follow the [Wishlists tutorial](https://guides.rubyonrails.org/wishlists.html) to continue learning.
Happy building!
