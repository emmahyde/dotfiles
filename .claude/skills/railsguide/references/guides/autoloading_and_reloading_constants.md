...
Autoload paths are called _root directories_ in Zeitwerk documentation, but we'll stay with "autoload path" in this guide.
...
You cannot autoload code in the autoload paths while the application boots. In particular, directly in `config/initializers/*.rb`. Please check _Autoloading when the application boots_ down below for valid ways to do that.
...
Reloading can be enabled or disabled. The setting that controls this behavior is [`config.enable_reloading`](https://guides.rubyonrails.org/configuring.html#config-enable-reloading), which is `true` by default in `development` mode, and `false` by default in `production` mode. For backwards compatibility, Rails also supports `config.cache_classes`, which is equivalent to `!config.enable_reloading`.
Rails uses an evented file monitor to detect files changes by default. It can be configured instead to detect file changes by walking the autoload paths. This is controlled by the [`config.file_watcher`](https://guides.rubyonrails.org/configuring.html#config-file-watcher) setting.
...
Eager loading is controlled by the flag [`config.eager_load`](https://guides.rubyonrails.org/configuring.html#config-eager-load), which is disabled by default in all environments except `production`. When a Rake task gets executed, `config.eager_load` is overridden by [`config.rake_eager_load`](https://guides.rubyonrails.org/configuring.html#config-rake-eager-load), which is `false` by default. So, by default, in production environments Rake tasks do not eager load the application.
