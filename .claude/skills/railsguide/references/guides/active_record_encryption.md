...
```
Copy
These values can be stored by copying and pasting the generated values into your existing [Rails credentials](https://guides.rubyonrails.org/security.html#custom-credentials) file using `bin/rails credentials:edit`.
...
The generated values are 32 bytes in length. If you generate these yourself, the recommended minimum length is 12 bytes for the primary key and 20 bytes for the [salt](https://en.wikipedia.org/wiki/Salt_\(cryptography\)).
...
The [`encrypts` method](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Encryption/EncryptableRecord.html#method-i-encrypts) defines the attributes to be encrypted at the model level. These are regular Active Record attributes backed by a column with the same name.
...
In non-deterministic mode, Active Record uses [AES](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard)-[GCM](https://en.wikipedia.org/wiki/Galois/Counter_Mode) with a 256-bits key and a random initialization vector. In deterministic mode, it also uses AES-GCM, but the initialization vector is not random. It is generated as a function of the key and the plaintext content ([HMAC](https://en.wikipedia.org/wiki/HMAC)-SHA-256 digest of the two).
...
If you have a mix of unencrypted and encrypted data or if you have data that is encrypted using two different sets of keys/schemes, you'll need to enable [extended queries](https://guides.rubyonrails.org/configuring.html#config-active-record-encryption-extend-queries) with `config.active_record.encryption.extend_queries = true` in order to support unique validations.
...
Encrypted attributes are configured to be automatically [filtered](https://guides.rubyonrails.org/configuring.html#config-filter-parameters) out of the Rails logs. So sensitive information, like encrypted emails or credit card numbers, isn't stored in your logs. For example, if you are filtering the `email` field, you will see something like this in the logs: `Parameters: {"email"=>"[FILTERED]", ...}`.
...
Compression is enabled by default but _not_ applied to all payloads. It is based on a size threshold (such as 140 bytes), which is used as a heuristic to determine if compression is "worth it".
...
With deterministic encryption, you typically want ciphertexts to remain constant. So when changing encryption schemes, non-deterministic and deterministic encryption behave differently.
  * With **non-deterministic encryption** , new information will always be encrypted with the _newest_ (current) encryption scheme.
  * With **deterministic encryption** , new information will be encrypted with the _oldest_ encryption scheme by default.

It is possible to change this behavior for deterministic encryption to use the _newest_ encryption scheme for encrypting new data like this:
...
```
Copy
You can use [`with_encryption_context`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Encryption/Contexts.html#method-i-with_encryption_context) to override any of the properties of the encryption context.
...
The [`DerivedSecretKeyProvider`](https://api.rubyonrails.org/v8.1.3/classes/ActiveRecord/Encryption/DerivedSecretKeyProvider.html) serves keys derived from the provided passwords using PBKDF2. This is the key provider configured by default.
...
In the example below, the _last key_ is used for encrypting new content and all keys are tried when decrypting content until one works.
