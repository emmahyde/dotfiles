...
### 3.1. Connections
For every WebSocket accepted by the server, a connection object is instantiated. This object becomes the parent of all the _channel subscriptions_ that are created from thereon. The connection itself does not deal with any specific application logic beyond authentication and authorization. The client of a WebSocket connection is called the connection _consumer_. An individual user will create one consumer-connection pair per browser tab, window, or device they have open.
Connections are instances of `ApplicationCable::Connection`, which extends [`ActionCable::Connection::Base`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Connection/Base.html). In `ApplicationCable::Connection`, you authorize the incoming connection and proceed to establish it if the user can be identified.
...
[`ActionCable::Connection::Callbacks`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Connection/Callbacks.html) provides callback hooks that are invoked when sending commands to the client, such as when subscribing, unsubscribing, or performing an action:

### 3.2. Channels
A _channel_ encapsulates a logical unit of work, similar to what a controller does in a typical MVC setup. By default, Rails creates a parent `ApplicationCable::Channel` class (which extends [`ActionCable::Channel::Base`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Channel/Base.html)) for encapsulating shared logic between your channels, when you use the channel generator for the first time.
...
#### 3.2.2. Subscriptions
Consumers subscribe to channels, acting as _subscribers_. Their connection is called a _subscription_. Produced messages are then routed to these channel subscriptions based on an identifier sent by the channel consumer.
...
As with `ApplicationCable::Connection`, you can also use [`rescue_from`](https://api.rubyonrails.org/v8.1.3/classes/ActiveSupport/Rescuable/ClassMethods.html#method-i-rescue_from) on a specific channel to handle raised exceptions:
...
[`ActionCable::Channel::Callbacks`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Channel/Callbacks.html) provides callback hooks that are invoked during the life cycle of a channel:
  * [`after_subscribe`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Channel/Callbacks/ClassMethods.html#method-i-after_subscribe) (aliased as [`on_subscribe`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Channel/Callbacks/ClassMethods.html#method-i-on_subscribe))
  * [`after_unsubscribe`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Channel/Callbacks/ClassMethods.html#method-i-after_unsubscribe) (aliased as [`on_unsubscribe`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Channel/Callbacks/ClassMethods.html#method-i-on_unsubscribe))
...
### 5.1. Streams
_Streams_ provide the mechanism by which channels route published content (broadcasts) to their subscribers. For example, the following code uses [`stream_from`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Channel/Streams.html#method-i-stream_from) to subscribe to the broadcasting named `chat_Best Room` when the value of the `:room` parameter is `"Best Room"`:
...
```
Copy
Then, elsewhere in your Rails application, you can broadcast to such a room by calling [`broadcast`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Server/Broadcasting.html#method-i-broadcast):
...
```
Copy
If you have a stream that is related to a model, then the broadcasting name can be generated from the channel and model. For example, the following code uses [`stream_for`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Channel/Streams.html#method-i-stream_for) to subscribe to a broadcasting like `posts:Z2lkOi8vVGVzdEFwcC9Qb3N0LzE`, where `Z2lkOi8vVGVzdEFwcC9Qb3N0LzE` is the GlobalID of the Post model.
...
```
Copy
You can then broadcast to this channel by calling [`broadcast_to`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Channel/Broadcasting/ClassMethods.html#method-i-broadcast_to):

```
PostsChannel.broadcast_to(@post, @comment)

### 5.2. Broadcastings
A _broadcasting_ is a pub/sub link where anything transmitted by a publisher is routed directly to the channel subscribers who are streaming that named broadcasting. Each channel can be streaming zero or more broadcastings.
...
A common use case is to _rebroadcast_ a message sent by one client to any other connected clients.
...
#### 6.1.1. Client-Server Interaction
  1. **Client** connects to the **Server** via `createConsumer()`. (`consumer.js`). The **Server** identifies this connection by `current_user`.
...
The Redis adapter requires users to provide a URL pointing to the Redis server. Additionally, a `channel_prefix` may be provided to avoid channel name collisions when using the same Redis server for multiple applications. See the [Redis Pub/Sub documentation](https://redis.io/docs/manual/pubsub/#database--scoping) for more details.
...
PostgreSQL has a [8000 bytes limit](https://www.postgresql.org/docs/current/sql-notify.html) on `NOTIFY` (the command used under the hood for sending notifications) which might be a constraint when dealing with large payloads.
...
To configure the URL, add a call to [`action_cable_meta_tag`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Helpers/ActionCableHelper.html#method-i-action_cable_meta_tag) in your HTML layout HEAD. This uses a URL or path typically set via [`config.action_cable.url`](https://guides.rubyonrails.org/configuring.html#config-action-cable-url) in the environment configuration files.
...
### 8.1. In App
Action Cable can run alongside your Rails application. For example, to listen for WebSocket requests on `/websocket`, specify that path to [`config.action_cable.mount_path`](https://guides.rubyonrails.org/configuring.html#config-action-cable-mount-path):
...
```
Copy
You can use `ActionCable.createConsumer()` to connect to the cable server if [`action_cable_meta_tag`](https://api.rubyonrails.org/v8.1.3/classes/ActionCable/Helpers/ActionCableHelper.html#method-i-action_cable_meta_tag) is invoked in the layout. Otherwise, a path is specified as first argument to `createConsumer` (e.g. `ActionCable.createConsumer("/websocket")`).
...
## 11. Testing
You can find detailed instructions on how to test your Action Cable functionality in the [testing guide](https://guides.rubyonrails.org/testing.html#testing-action-cable).
