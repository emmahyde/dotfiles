...
### 1.4. The Update Task
Rails provides the `bin/rails app:update` command. After updating the Rails version in the `Gemfile`, run this command. This will help you with the creation of new files and changes of old files in an interactive session.
...
### 1.5. Configure Framework Defaults
The new Rails version might have different configuration defaults than the previous version. However, after following the steps described above, your application would still run with configuration defaults from the _previous_ Rails version. That's because the value for `config.load_defaults` in `config/application.rb` has not been changed yet.
...
## 2. Upgrading from Rails 8.0 to Rails 8.1
...
### 2.1. The table columns inside `schema.rb` are now sorted alphabetically.
...
## 3. Upgrading from Rails 7.2 to Rails 8.0
...
## 4. Upgrading from Rails 7.1 to Rails 7.2
...
### 4.1. All tests now respect the `active_job.queue_adapter` config
If you have set `config.active_job.queue_adapter` in your `config/application.rb` or `config/environments/test.rb` file, the adapter you selected was previously not used consistently across all tests. In some tests your adapter would be used, but other tests would use the `TestAdapter`.
In Rails 7.2, all tests will respect the `queue_adapter` config if provided. This may cause test errors, if you had set the `queue_adapter` config to something other than `:test`, but written tests in a way that was dependent on the `TestAdapter`.
If no config is provided, the `TestAdapter` will continue to be used.

### 4.2. `alias_attribute` now bypasses custom methods on the original attribute
...
## 5. Upgrading from Rails 7.0 to Rails 7.1
...
### 5.1. Development and test environments secret_key_base file changed
...
### 5.2. New ActiveSupport::Cache serialization format
...
### 5.3. Autoloaded paths are no longer in $LOAD_PATH
Starting from Rails 7.1, the directories managed by the autoloaders are no longer added to `$LOAD_PATH`. This means it won't be possible to load their files with a manual `require` call, which shouldn't be done anyway.
Reducing the size of `$LOAD_PATH` speeds up `require` calls for apps not using `bootsnap`, and reduces the size of the `bootsnap` cache for the others.
...
```
Copy
but we discourage doing so, classes and modules in the autoload paths are meant to be autoloaded. That is, just reference them.
The `lib` directory is not affected by this flag, it is added to `$LOAD_PATH` always.

### 5.4. config.autoload_lib and config.autoload_lib_once
If your application does not have `lib` in the autoload or autoload once paths, please skip this section. You can find that out by inspecting the output of
...
```
Copy
If your application already has `lib` in the autoload paths, normally there is configuration in `config/application.rb` that looks something like
...
```
Copy
That still works, but it is recommended to replace those lines with the more concise
...
```
Copy
Please, add to the `ignore` list any other `lib` subdirectories that do not contain `.rb` files, or that should not be reloaded or eager loaded. For example, if your application has `lib/templates`, `lib/generators`, or `lib/middleware`, you'd add their name relative to `lib`:
...
```
Copy
With that one-liner, the (non-ignored) code in `lib` will be also eager loaded if `config.eager_load` is `true` (the default in `production` mode). This is normally what you want, but if `lib` was not added to the eager load paths before and you still want it that way, please opt-out:
...
```
Copy
The method `config.autoload_lib_once` is the analogous one if the application had `lib` in `config.autoload_once_paths`.

### 5.5. `ActiveStorage::BaseController` no longer includes the streaming concern
...
### 5.6. `MemCacheStore` and `RedisCacheStore` now use connection pooling by default
The `connection_pool` gem has been added as a dependency of the `activesupport` gem, and the `MemCacheStore` and `RedisCacheStore` now use connection pooling by default.
If you don't want to use connection pooling, set `:pool` option to `false` when configuring your cache store:
...
### 5.7. `SQLite3Adapter` now configured to be used in a strict strings mode
...
### 5.8. Support multiple preview paths for `ActionMailer::Preview`
...
### 5.9. `config.i18n.raise_on_missing_translations = true` now raises on any missing translation.
...
```
Copy
Alternatively, you can customize the `I18n.exception_handler`. See the [i18n guide](https://guides.rubyonrails.org/v7.1/i18n.html#using-different-exception-handlers) for more information.
`AbstractController::Translation.raise_on_missing_translations` has been removed. This was a private API, if you were relying on it you should migrate to `config.i18n.raise_on_missing_translations` or to a custom exception handler.

### 5.10. `bin/rails test` now runs `test:prepare` task
...
### 5.11. Import syntax from `@rails/ujs` is modified
Starting from Rails 7.1, the syntax for importing modules from `@rails/ujs` is modified. Rails no longer supports the direct import of a module from `@rails/ujs`.
...
```
Copy
In Rails 7.1, users should first import the Rails object directly from `@rails/ujs`. Users can then import specific modules from the Rails object.
...
### 5.12. `Rails.logger` now returns an `ActiveSupport::BroadcastLogger` instance
...
The API to broadcast logs (using the `ActiveSupport::Logger.broadcast` method) was removed and was previously private. If your application or library was relying on this API, you need to make the following changes:
...
### 5.13. Active Record Encryption algorithm changes
Active Record Encryption now uses SHA-256 as its hash digest algorithm. If you have data encrypted with previous Rails versions, there are two scenarios to consider:
  1. If you have `config.active_support.key_generator_hash_digest_class` configured as SHA-1 (the default before Rails 7.0), you need to configure SHA-1 for Active Record Encryption too:
...
```
Copy
  2. If you have `config.active_support.key_generator_hash_digest_class` configured as SHA-256 (the new default in 7.0), then you need to configure SHA-256 for Active Record Encryption:

```
config.active_record.encryption.hash_digest_class = OpenSSL::Digest::SHA256

See the [Configuring Rails Applications](https://guides.rubyonrails.org/configuring.html#config-active-record-encryption-hash-digest-class) guide for more information on `config.active_record.encryption.hash_digest_class`.
In addition, a new configuration [`config.active_record.encryption.support_sha1_for_non_deterministic_encryption`](https://guides.rubyonrails.org/configuring.html#config-active-record-encryption-support-sha1-for-non-deterministic-encryption) was introduced to resolve [a bug](https://github.com/rails/rails/issues/42922) that caused some attributes to be encrypted using SHA-1 even when SHA-256 was configured via the aforementioned `hash_digest_class` configuration.
By default, `config.active_record.encryption.support_sha1_for_non_deterministic_encryption` is disabled in Rails 7.1. If you have data encrypted in a version of Rails < 7.1 that you believe may be affected by the aforementioned bug, this configuration should be enabled:

```
Copy
**If you are working with encrypted data, please carefully review the above.**

### 5.14. New ways to handle exceptions in Controller Tests, Integration Tests, and System Tests
...
## 6. Upgrading from Rails 6.1 to Rails 7.0
...
### 6.1. `ActionView::Helpers::UrlHelper#button_to` changed behavior
...
```
Copy
Also, make sure [`config.cache_classes`](https://guides.rubyonrails.org/configuring.html#config-cache-classes) is set to `false` in `config/environments/test.rb`.

### 6.3. Sprockets is now an optional dependency
...
### 6.4. Applications need to run in `zeitwerk` mode
...
### 6.5. The setter `config.autoloader=` has been deleted
...
### 6.6. `ActiveSupport::Dependencies` private API has been deleted
...
### 6.7. Autoloading during initialization
...
### 6.8. Ability to configure `config.autoload_once_paths`
[`config.autoload_once_paths`](https://guides.rubyonrails.org/configuring.html#config-autoload-once-paths) can be set in the body of the application class defined in `config/application.rb` or in the configuration for environments in `config/environments/*`.
...
### 6.9. `ActionDispatch::Request#content_type` now returns Content-Type header as it is.
...
### 6.10. Key generator digest class change requires a cookie rotator
...
In order to be able to read messages using the old digest class it is necessary to register a rotator. Failing to do so may result in users having their sessions invalidated during the upgrade.
...
### 6.11. Digest class for ActiveSupport::Digest changing to SHA256
...
### 6.12. New ActiveSupport::Cache serialization format
...
### 6.13. Active Storage video preview image generation
...
### 6.14. Active Storage default variant processor changed to `:vips`
...
#### 6.14.1. Replace resize with resize_to_limit
...
#### 6.14.2. Use an array when cropping
...
#### 6.14.4. Adjust the background color used for `resize_and_pad`
...
#### 6.14.5. Remove any EXIF based rotation
...
#### 6.14.6. Replace monochrome with colourspace
...
#### 6.14.7. Switch to libvips options for compressing images
JPEG
...
### 6.15. Rails version is now included in the Active Record schema dump
...
Before loading the schema for the first time in Rails 7.0, make sure to run `bin/rails app:update` to ensure that the version of the schema is included in the schema dump.
...
## 7. Upgrading from Rails 6.0 to Rails 6.1
...
### 7.1. `Rails.application.config_for` return value no longer supports access with String keys.
...
### 7.2. Response's Content-Type when using `respond_to#any`
...
```
Copy
Previous behavior was returning a `text/csv` response's Content-Type which is inaccurate since a JSON response is being rendered. Current behavior correctly returns a `application/json` response's Content-Type.
If your application relies on the previous incorrect behavior, you are encouraged to specify which formats your action accepts, i.e.

```
format.any(:xml, :json) { render request.format.to_sym => @people }

### 7.3. `ActiveSupport::Callbacks#halted_callback_hook` now receive a second argument
...
### 7.4. The `helper` class method in controllers uses `String#constantize`
...
### 7.5. Redirection to HTTPS from HTTP will now use the 308 HTTP status code
...
### 7.6. Active Storage now requires Image Processing
...
### 7.7. New `ActiveModel::Error` class
Errors are now instances of a new `ActiveModel::Error` class, with changes to the API. Some of these changes may throw errors depending on how you manipulate errors, while others will print deprecation warnings to be fixed for Rails 7.0.
More information about this change and details about the API changes can be found [in this PR](https://github.com/rails/rails/pull/32313).

## 8. Upgrading from Rails 5.2 to Rails 6.0
...
### 8.2. Force SSL
The `force_ssl` method on controllers has been deprecated and will be removed in Rails 6.1. You are encouraged to enable [`config.force_ssl`](https://guides.rubyonrails.org/configuring.html#config-force-ssl) to enforce HTTPS connections throughout your application. If you need to exempt certain endpoints from redirection, you can use [`config.ssl_options`](https://guides.rubyonrails.org/configuring.html#config-ssl-options) to configure that behavior.

### 8.3. Purpose and expiry metadata is now embedded inside signed and encrypted cookies for increased security
...
### 8.4. All npm packages have been moved to the `@rails` scope
...
### 8.5. Action Cable JavaScript API Changes
...
### 8.6. `ActionDispatch::Response#content_type` now returns the Content-Type header without modification
...
### 8.7. New `config.hosts` setting
Rails now has a new `config.hosts` setting for security purposes. This setting defaults to `localhost` in development. If you use other domains in development you need to allow them like this:
...
```
Copy
For other environments `config.hosts` is empty by default, which means Rails won't validate the host at all. You can optionally add them if you want to validate it in production.
...
#### 8.8.4. Qualified names in class and module definitions
...
#### 8.8.6. Having `app` in the autoload paths
...
#### 8.8.7. Autoloaded Constants and Explicit Namespaces
...
#### 8.8.8. One file, one constant (at the same top-level)
...
#### 8.8.9. Spring and the `test` Environment
...
The new configuration point [`config.add_autoload_paths_to_load_path`](https://guides.rubyonrails.org/configuring.html#config-add-autoload-paths-to-load-path) is `true` by default for backwards compatibility, but allows you to opt-out from adding the autoload paths to `$LOAD_PATH`.
...
#### 8.8.13. Globs in config.autoload_paths
...
#### 8.8.14. Eager loading and autoloading are consistent
...
#### 8.8.15. How to Use the Classic Autoloader in Rails 6
...
### 8.9. Active Storage assignment behavior change
...
```
Copy
Existing applications can opt in to this new behavior by setting [`config.active_storage.replace_on_assign_to_many`](https://guides.rubyonrails.org/configuring.html#config-active-storage-replace-on-assign-to-many) to `true`. The old behavior will be deprecated in Rails 7.0 and removed in Rails 7.1.

### 8.10. Custom exception handling applications
Invalid `Accept` or `Content-Type` request headers will now raise an exception. The default [`config.exceptions_app`](https://guides.rubyonrails.org/configuring.html#config-exceptions-app) specifically handles that error and compensates for it. Custom exceptions applications will need to handle that error as well, or such requests will cause Rails to use the fallback exceptions application, which returns a `500 Internal Server Error`.

## 9. Upgrading from Rails 5.1 to Rails 5.2
...
### 9.1. Bootsnap
Rails 5.2 adds bootsnap gem in the [newly generated app's Gemfile](https://github.com/rails/rails/pull/29313). The `app:update` command sets it up in `boot.rb`. If you want to use it, then add it in the Gemfile:
...
### 9.2. Expiry in signed or encrypted cookie is now embedded in the cookies values
...
## 10. Upgrading from Rails 5.0 to Rails 5.1
...
### 10.1. Top-level `HashWithIndifferentAccess` is soft-deprecated
...
### 10.2. `application.secrets` now loaded with all keys as symbols
...
### 10.3. Removed deprecated support to `:text` and `:nothing` in `render`
...
### 10.4. Removed deprecated support of `redirect_to :back`
...
### 11.2. Active Record Models Now Inherit from ApplicationRecord by Default
...
### 11.3. Halting Callback Chains via `throw(:abort)`
...
### 11.4. ActiveJob Now Inherits from ApplicationJob by Default
...
#### 11.5.1. Extraction of some helper methods to `rails-controller-testing`
...
#### 11.5.2. New behavior when uploading files
...
### 11.6. Autoloading is Disabled After Booting in the Production Environment
...
### 11.8. Removed Support for Legacy `mysql` Database Adapter
...
### 11.11. `ActionController::Parameters` No Longer Inherits from `HashWithIndifferentAccess`
...
### 11.12. `protect_from_forgery` Now Defaults to `prepend: false`
...
### 11.13. Default Template Handler is Now RAW
...
### 11.14. Added Wildcard Matching for Template Dependencies
...
### 11.15. `ActionView::Helpers::RecordTagHelper` moved to external gem (record_tag_helper)
...
### 11.16. Removed Support for `protected_attributes` Gem
The `protected_attributes` gem is no longer supported in Rails 5.

### 11.17. Removed support for `activerecord-deprecated_finders` gem
The `activerecord-deprecated_finders` gem is no longer supported in Rails 5.

### 11.18. `ActiveSupport::TestCase` Default Test Order is Now Random
...
### 11.19. `ActionController::Live` became a `Concern`
...
### 11.20. New Framework Defaults

#### 11.20.1. Active Record `belongs_to` Required by Default Option
...
#### 11.20.3. Forgery Protection with Origin Check
...
#### 11.20.4. Allow Configuration of Action Mailer Queue Name
...
#### 11.20.5. Support Fragment Caching in Action Mailer Views
Set [`config.action_mailer.perform_caching`](https://guides.rubyonrails.org/configuring.html#config-action-mailer-perform-caching) in your config to determine whether your Action Mailer views should support caching.

```
config.action_mailer.perform_caching = true

#### 11.20.6. Configure the Output of `db:structure:dump`
...
## 12. Upgrading from Rails 4.1 to Rails 4.2
...
### 12.3. Error handling in transaction callbacks
...
### 12.7. `after_bundle` in Rails templates
...
### 13.1. CSRF protection from remote `<script>` tags
...
### 13.7. Changes in JSON handling
There are a few major changes related to JSON handling in Rails 4.1.

#### 13.7.1. MultiJSON removal
...
Do not simply replace `MultiJson.load` with `JSON.load` as that API is meant for deserializing arbitrary Ruby objects and is generally [unsafe](https://docs.ruby-lang.org/en/master/JSON.html#method-i-load). Prefer to use `JSON.parse`.

#### 13.7.2. JSON gem compatibility
...
#### 13.7.4. JSON representation of Time objects
...
### 13.8. Usage of `return` within inline callback blocks
...
### 13.9. Methods defined in Active Record fixtures
...
### 13.10. I18n enforcing available locales
...
To disable it (and allow I18n to accept _any_ locale option) add the following configuration to your application:
...
### 13.11. Mutator methods called on Relation
...
### 13.12. Changes on Default Scopes
...
### 13.13. Rendering content from string
Rails 4.1 introduces `:plain`, `:html`, and `:body` options to `render`. Those options are now the preferred way to render string-based content, as it allows you to specify which content type you want the response sent as.
  ...
  * `render :body` will _not_ set the content type header.
...
### 13.14. PostgreSQL JSON and hstore datatypes
...
### 13.15. Explicit block use for `ActiveSupport::Callbacks`
...
```
Copy
For more on PATCH and why this change was made, see [this post](https://rubyonrails.org/2012/2/26/edge-rails-patch-is-the-new-primary-http-method-for-updates) on the Rails blog.

#### 14.1.1. A note about media types
...
Please note that if your application is dependent on loading certain pages in a `<frame>` or `<iframe>`, then you may need to explicitly set `X-Frame-Options` to `ALLOW-FROM ...` or `ALLOWALL`.
  * In Rails 4.0, precompiling assets no longer automatically copies non-JS/CSS assets from `vendor/assets` and `lib/assets`. Rails application and engine developers should put these assets in `app/assets` or configure [`config.assets.precompile`](https://guides.rubyonrails.org/configuring.html#config-assets-precompile).
...
### 14.11. sprockets-rails
  ...
  * The `config.assets.compress` option should be changed to [`config.assets.js_compressor`](https://guides.rubyonrails.org/configuring.html#config-assets-js-compressor) like so for instance:
...
## 15. Upgrading from Rails 3.1 to Rails 3.2
...
## 16. Upgrading from Rails 3.0 to Rails 3.1
...
### 16.8. Remove :cache and :concat options in asset helpers references in views
