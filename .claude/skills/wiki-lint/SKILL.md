...
1. **Orphan pages**. Wiki pages with no inbound wikilinks. They exist but nothing points to them. See **Scan Scope** below for which paths participate as orphan candidates.
2. **Dead links**. Wikilinks that reference a target that does not exist. Resolution scope includes `wiki/**/*.md`, `wiki/**/*.canvas`, AND `.raw/` if the vault uses cross-zone pointers. See **Link Resolution Scope** below.
...
The lint scan operates on two distinct path sets:

**Orphan-candidate set** — pages that must have at least one inbound wikilink to avoid being flagged as orphans. Excludes:

- Anything under `wiki/_templates/` (not actual content).
- Anything under `wiki/meta/` (lint reports, dashboards, canvas maps — these are observational artifacts; orphan status is expected).
- Anything under `wiki/folds/` (DragonScale folds carry their own `fold_id` and intentionally float).
- Filename in `{_index.md, index.md, log.md, hot.md, overview.md, dashboard.md, dashboard.base, Wiki Map.md, getting-started.md}`.
- Pages with `type: meta` or `type: fold` in frontmatter.

**Link-target set** — what a `[[Target]]` wikilink can resolve to without being flagged as dead. Includes:

- Every `.md` under `wiki/` (basename match, case-sensitive).
- Every `.canvas` under `wiki/` (basename match — canvas files are valid wikilink targets in Obsidian).
- Every `.md` under `.raw/` if the vault keeps `[[notion/foo-YYYY-MM-DD]]` style cross-zone pointers. Strip the leading `notion/` / `skills/` / `jira/` / `slack/` / `articles/` / `github/` etc. and match against the `.raw/` tree.
- If a wikilink contains a `/`, resolve by full path under `wiki/` first, then under `.raw/`. If neither hits, fall back to basename match.

Pages that match no path in the link-target set are flagged dead.

---

## Wikilink Extraction (regex hygiene)

Before applying the wikilink regex, scrub the page body of code:

1. Strip fenced code blocks (triple-backtick or tilde, including language tags): match each fence line by `^\s*(```|~~~)` and toggle in/out state.
2. Strip **inline code** spans on every non-fenced line: replace `` `[^`]*` `` with empty string before applying the wikilink regex.
3. Then apply the wikilink regex `\[\[([^\]|#]+?)(?:\|[^\]]+)?(?:#[^\]]+)?\]\]` to the scrubbed body.

Without inline-code scrubbing, tuple literals like `` `[true, nil, nil]` `` or array syntax inside backticks get parsed as wikilink targets, producing false-positive dead-link reports against expressions that are not links at all.

Sanity check on extracted targets:

- If the target contains a comma, it is almost certainly not a wikilink. Reject.
- If the target contains a newline, reject.
- Leading/trailing whitespace inside `[[...]]` is significant only to author intent — trim before matching.
